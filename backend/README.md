...
## Приложения (Timetable / Control Panel)

- **Timetable (Frontend)** — при первом запуске сам запускает бэкенд (sidecar) на порту 8000. БД и медиафайлы хранятся в папке данных приложения (AppData и т.п.), поэтому при обновлении приложения данные сохраняются.
- **Timetable Control Panel (Admin)** — подключается к уже запущенному бэкенду. В разделе «База данных» можно скачать резервную копию (.db) или загрузить ранее сохранённую базу, чтобы не заполнять данные заново. Запустите сначала приложение Timetable или бэкенд вручную (`uvicorn main:app --host 0.0.0.0 --port 8000`).
